...
def CliInit():
    ...
    parser.add_argument(
       ...
       example: {fg.BYELLOW}filewarp --convert_doc example.docx -tff pdf{RESET}",
    )

    parser.add_argument(
        "--convert_audio",
        help=f"Convert audio file(s) to and from different format ie mp3 to wav\
        example: {fg.BYELLOW}filewarp --convert_audio example.mp3 -tff wav{RESET}",
    )

    parser.add_argument(
        "--convert_video",
        help=f"Convert video file(s) to and from different format ie mp4 to mkv.\
        example: {fg.BYELLOW}filewarp --convert_video example.mp4 -tf mkv{RESET}",
    )

    parser.add_argument(
        "--convert_image",
        help=f"Convert image file(s) to and from different format ie png to jpg.\
        example: {fg.BYELLOW}filewarp --convert_image example.jpg -tf png{RESET}",
    )

    parser.add_argument(
        "--convert_svg",
        help=f"Converter svg file(s) to different format ie pdf, png.\
       example: {fg.BYELLOW}filewarp --convert_svg example.svg -tff pdf{RESET}",
    )

    parser.add_argument(
        "--convert_doc2image",
        help=f"Convert documents to images ie png to jpg.\
        example: {fg.BYELLOW}filewarp --convert_doc2image example.pdf -tf png{RESET}",
    )

    parser.add_argument(
        "--html2word",
        help=f"Convert image file(s) to and from different format ie png to jpg.\
        example: {fg.BYELLOW}filewarp --html2word index.html{RESET}",
    )
    ...
    parser.add_argument(
        ...
        help=f"Convert Markdown to DOCX with Mermaid rendering.\
            example: {fg.BYELLOW}filewarp --markdown2docx example.md{RESET}",
    )
    parser.add_argument(
        "-xA",
        "--extract_audio",
        help=f"Extract audio from a video.\
                        example: {fg.BYELLOW}filewarp -xA example.mp4 {RESET}",
    )
    ...
    parser.add_argument(
        ...
        help=f"Specify file types to isolate\
                        for conversion, only works if directory is provided as input for the {fg.FCYAN}convert_doc{RESET} argument example: {fg.BYELLOW}filewarp --convert_doc /home/user/Documents/ --isolate pdf -tf txt{RESET}",
    )
    ...
    parser.add_argument(
        ...
        example: {fg.BYELLOW}filewarp --analyze_video example.mp4 {RESET}",
    )
    ...
    parser.add_argument(
        ...
        example: {fg.BYELLOW}filewarp --resize_image example.png -tf_size 2mb -tf png {RESET}",
    )
    ...
    parser.add_argument(
        ...
        help=f"Scan pdf file and extract text\
                        example: {fg.BYELLOW}filewarp --scan example.pdf {RESET}",
    )
    ...
    parser.add_argument(
        ...
        help=f"Convert pdf file to long image\
                        example: {fg.BYELLOW}filewarp --doc_long_image example.pdf {RESET}",
    )
    ...
    parser.add_argument(
        ...
        help=f"Convert pdf to image then extract text\
                        example: {fg.BYELLOW}filewarp --scanAsImg example.pdf {RESET}",
    )
    ...
    parser.add_argument(
        ...
        example: {fg.BYELLOW}filewarp --OCR image.png{RESET}",
    )
    ...
    parser.add_argument(
        ...
        help=f"Advanced Text to word conversion i.e:{
            fg.BYELLOW
        }filewarp --Atext2word example.txt --font_size 12 --font_name Arial{RESET}",
    )
    ...
    parser.add_argument(
        ...
        help=f"Use alternative conversion method: Overides\
                        default method i.e: {fg.BYELLOW}filewarp --convert_doc example.docx --use_extras -tf pdf{RESET}",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="AAB",
        help=f"Order of pages when joining the pdf use: {fg.BYELLOW}filewarp\
    ...
    parser.add_argument(
        ...
        help=f"Extract given pages from pdf: {
            fg.BYELLOW
        }filewarp --extract_pages file.pdf 6 10{RESET} for one page: {
            fg.BYELLOW
        }filewarp --extract_pages file.pdf 5{RESET}",
    )
    ...
    parser.add_argument(
        ...
        action="store_false",
        dest="no_resume",
        help=f"Don't Resume previous File operation {fg.BYELLOW}filewarp --convert_doc simpledir --no-resume{RESET}",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help=f"Number of threads for text to speech  {fg.BYELLOW}filewarp --convert_doc simpledir --no-resume -t 2{RESET}",
    )
    parser.add_argument(
        "-sep",
        "--separator",
    ...
    parser.add_argument(
        "--image2pdf",
        nargs="+",
        help=f"Convert Images to pdf. {fg.BWHITE}Accepts image list or dir/folder{RESET} e.g `{fg.BYELLOW}filewarp --image2pdf image1 image2{RESET}`",
    )
    parser.add_argument(
        "--image2word",
        nargs="+",
        help=f"Convert Images to word document. {fg.BWHITE}Accepts image list or dir/folder{RESET} e.g `{fg.BYELLOW}filewarp --image2word image1 image2{RESET}`",
    )
    parser.add_argument(
        "--image2gray",
        nargs="+",
        help=f"Convert Images to grayscale. {fg.BWHITE}Accepts image list or dir/folder{RESET} e.g `{fg.BYELLOW}filewarp --image2gray image1 image2{RESET}`",
    )
    parser.add_argument(
        "-vt",
        "--voicetype",
        action="store_true",
        help=f"Use your voice to type text. e.g `{fg.BYELLOW}filewarp --voicetype{RESET}`",
    )
    ...
    parser.add_argument(
        ...
        "--extract_img",
        nargs="+",
        help=f"Extract images from pdf. e.g `{fg.BYELLOW}filewarp --extract_img input.pdf{RESET}`",
    )
...
class OperationMapper:
    ...
    def display_version(self):
        ...
        return print(f"{fg.BLUE}filewarp: V-{fg.BGREEN}{version}{RESET}")
