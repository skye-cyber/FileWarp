...
def cli(ctx, version, no_resume, threads):
    ...
    if ctx.invoked_subcommand is None:
        display_banner()
        console.print(
            "\n[bold]Welcome to Filemac![/] Use [cyan]filemac --help[/] for commands.\n"
        )
        show_quick_commands()
...
def show_quick_commands():
    ...
    commands = [
        ("convert-doc", "Convert documents", "filemac convert-doc file.docx --to pdf"),
        (
            "convert-audio",
            "Convert audio files",
            "filemac convert-audio song.mp3 --to wav",
        ),
        ("convert-video", "Convert videos", "filemac convert-video video.mp4 --to mkv"),
        ("convert-image", "Convert images", "filemac convert-image photo.jpg --to png"),
        ("ocr", "Extract text from images", "filemac ocr image.png"),
        ("pdf-join", "Join PDF files", "filemac pdf-join file1.pdf file2.pdf"),
        ("scan", "Scan PDF for text", "filemac scan document.pdf"),
    ]
